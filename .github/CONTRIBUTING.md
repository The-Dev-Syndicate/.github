...
*Don't worry if your code doesn't make it in on the first try; you will still receive valuable feedback and have the opportunity to grow!*
...
2. Pull the latest changes:
   ...
   ```
3. Create a new branch for your work using the format:

   ```sh
   {issue_number}-{custom-branch-name}
   ```

   For example, if you are working on issue number 5 and want to call your branch `fix-login-error`, you would create:

   ```sh
   git checkout -b 5-fix-login-error
   ```

### Making Changes

1. Make your changes in the new branch.
2. Commit your changes with a clear and concise commit message linking to the issue:

   ```sh
   git add .
   git commit -m "[#5] - Add description of the changes made"
   ```

   *This helps us trace commits back to issues in case we need to do a rollback.*
...
* Please title your PR using the convention: `[#issue_number] - {Short Title}`.

  * Example: `[#5] - Fix login error`
* All pull requests (PRs) must link to the issue they are closing using the keywords "closes #{issue_number}":
...
Example: A branch for issue number 5 fixing a login error would be:
